...
ObjectTextDisplay::ObjectTextDisplay(const Ogre::MovableObject* p, const Ogre::Camera* c, const Ogre::ColourValue colourValue) {
	m_p = p;
	m_c = c;
	m_enabled = false;
	m_text = "";
	...
	m_pOverlay = Ogre::OverlayManager::getSingleton().create(p->getName()+"OverLay");
	m_pContainer = static_cast<Ogre::OverlayContainer*>(Ogre::OverlayManager::getSingleton().createOverlayElement(
                "Panel", p->getName()+"Container"));

	m_pOverlay->add2D(m_pContainer);

	m_pText = Ogre::OverlayManager::getSingleton().createOverlayElement("TextArea", p->getName()+"OverLayText");
...
ObjectTextDisplay::~ObjectTextDisplay() {
	...
	m_pContainer->removeChild(m_p->getName()+"OverLayText");
