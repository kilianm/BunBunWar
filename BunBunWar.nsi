...
Section "Source"
  ...
  ; Put file there
  File /r /x redist /x bin /x media /x bin-debug /x .svn /x Release /x Debug /x *.ncb /x setup.exe /x *.nsi /x *.log *
...
Section "Uninstall"
  ...
  RMDir /r $INSTDIR\OgreGODEngine
  RMDir /r $INSTDIR\OgreLayer
  RMDir /r $INSTDIR\Utilities
  RMDir /r $INSTDIR\BunBunWar
  RMDir /r $INSTDIR\GOD-Engine
  RMDir /r $INSTDIR\GOD-Tester
  Delete $INSTDIR\*.*
